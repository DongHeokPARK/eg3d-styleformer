...
def setup_training_loop_kwargs(
    ...
    num_layers   = None, # Number of transformer encoder_layer: <list>, default = [1,3,3]
    g_dict       = None, # Hidden dimension of transformer encoder_layer for resolution, default = [256,64,16]
    depth        = None, # Depth of transformer encoder_layer, default = 32
    minimum_head = None, # Minimum num head of transformer encoder_layer, default = 1'
    linformer    = None, # Use linformer : <bool>, default = False
):
    ...
    args.num_layers = list(map(lambda x: int(x), num_layers))
    
    if g_dict is None:
        g_dict = [256,64,16]
    assert isinstance(g_dict, list)
    args.G_dict = list(map(lambda x: int(x), g_dict))
    ...
    if minimum_head is None:
        minimum_head = 1
    assert isinstance(minimum_head, int)
    args.minimum_head = minimum_head
    
    if linformer is None:
        linformer = False
    assert isinstance(linformer, bool)
    args.linformer = linformer
    ...
    cfg_specs = {
        ...
        'cifar':     dict(ref_gpus=1,  kimg=100000, mb=32, mbstd=32, fmaps=1,   lrate=0.0025, gamma=0.01, ema=500, ramp=0.05, map=2),
    }
    ...
    if batch is not None:
        ...
        args.D_kwargs.epilogue_kwargs.mbstd_group_size = min(args.batch_gpu, 4)
...
@click.option('--depth', help='Depth of transformer encoder_layer', type=int, metavar='INT')
@click.option('--minimum_head', help='Minimum head of transformer encoder_layer', type=int, metavar='INT')
